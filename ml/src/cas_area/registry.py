...
def default_model_path(models_dir: Path, model_version: str = MODEL_VERSION) -> Path:
    ...
    Takes the directory itself rather than a project root: the serving code lives
    in a standalone sibling project, so there is no shared root to resolve against.
    """
